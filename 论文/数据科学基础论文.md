...
PCA最常见的应用之一是将高维数据集可视化。我们以sklearn中最常见的Iris数据集为例，Iris数据集包含150个样本，分为3类，每个样本具有四个特征。
由于样本特征数大于2已属于高维数据，我们可以使用PCA进行降维，使样本数只包含两个新特征。（具体代码实现位于GitHub仓库中3-2机器学习sk.py）

![Iris PCA](asset/Iris%20PCA.png)

#### 案例2 处理64维数字图像，将其降到2维
数据集来源： https://archive.ics.uci.edu/ml/machine-learning-databases/optdigits/optdigits.tra
该数据集有3000多个样本，可分为10类，每个样本有64个特征。
PCA在处理64维的高维数据时可以将其直接降到2维，可视化成图表后给人以直观的感受。

![64 ](asset/opdigits.png)
